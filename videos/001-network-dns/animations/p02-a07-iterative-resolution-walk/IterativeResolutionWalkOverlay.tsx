import React from 'react';
import {interpolate, useCurrentFrame, useVideoConfig} from 'remotion';
import {clamp, msToFrames, smoothProgress} from '@channel/motion-core';
import {DnsPresenterOverlayChrome} from '../../shared/DnsPresenterOverlayChrome';
import {DnsSceneHeading} from '../../shared/DnsSceneHeading';
import {DnsSceneStatus} from '../../shared/DnsSceneStatus';
import {getActiveTimedPhaseIndex} from '../../shared/getActiveTimedPhaseIndex';
import {DnsTraversalMap} from './DnsTraversalMap';
import {traversalPhases} from './content';

export const IterativeResolutionWalkOverlay: React.FC = () => {
  const frame = useCurrentFrame();
  const {durationInFrames, fps} = useVideoConfig();
  const elapsedMs = (frame / fps) * 1000;
  const reveal = smoothProgress(frame, 4, 18);
  const activeIndex = getActiveTimedPhaseIndex(traversalPhases, elapsedMs);
  const phase = traversalPhases[activeIndex] ?? traversalPhases[0];
  const phaseProgress = smoothProgress(
    frame,
    msToFrames(phase.startMs, fps),
    msToFrames(phase.startMs + 760, fps),
  );
  const answerReveal = smoothProgress(
    frame,
    msToFrames(46_700, fps),
    msToFrames(48_100, fps),
  );
  const runtimeProgress = interpolate(frame, [0, durationInFrames - 1], [0, 1], clamp);

  return (
    <div style={{position: 'absolute', inset: 0, overflow: 'hidden'}}>
      <DnsPresenterOverlayChrome
        label="ИТЕРАТИВНЫЙ ОБХОД"
        meta="ROOT / TLD / ZONE / RR"
        progress={runtimeProgress}
      />
      <DnsSceneHeading
        accent={phase.accent}
        eyebrow={phase.eyebrow}
        reveal={phaseProgress}
        title={phase.title}
      />
      <DnsTraversalMap
        answerReveal={answerReveal}
        direction={phase.direction}
        level={phase.level}
        phaseProgress={phaseProgress}
        reveal={reveal}
      />
      <DnsSceneStatus
        accent={phase.accent}
        left={phase.status}
        right={`DNS / 009 / ${String(activeIndex + 1).padStart(2, '0')}`}
        reveal={reveal}
      />
    </div>
  );
};
