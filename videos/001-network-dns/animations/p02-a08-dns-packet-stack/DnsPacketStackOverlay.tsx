...
import {packetStackPhases} from './content';
import {PacketStackDiagram} from './PacketStackDiagram';

export const DnsPacketStackOverlay: React.FC = () => {
  const frame = useCurrentFrame();
  const {durationInFrames, fps} = useVideoConfig();
  const elapsedMs = (frame / fps) * 1000;
  const reveal = smoothProgress(frame, 4, 18);
  const activeIndex = getActiveTimedPhaseIndex(packetStackPhases, elapsedMs);
  const phase = packetStackPhases[activeIndex] ?? packetStackPhases[0];
  ...
  return (
    <div style={{position: 'absolute', inset: 0, overflow: 'hidden'}}>
      ...
      />
      <PacketStackDiagram
        activeLayer={phase.activeLayer}
        envelopeDepth={phase.envelopeDepth}
      ...
      <DnsSceneStatus
        accent={phase.accent}
        left={phase.status}
        right={`DNS / 010 / ${String(activeIndex + 1).padStart(2, '0')}`}
        reveal={reveal}
      />
    </div>
  );
